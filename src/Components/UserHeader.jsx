...
import { faHome, faUser, faClipboardList, faSignOutAlt, faBars, faTimes, faDroplet, faHandHoldingHeart, faMap } from '@fortawesome/free-solid-svg-icons';
...
const UserHeader = () => {
  ...
  const menuItems = [
    ...
    { icon: faDroplet, label: 'Registered as donor', path: '/pdonor' },
    { icon: faHandHoldingHeart, label: 'Registered as acceptor', path: '/pacceptor' },
    { icon: faClipboardList, label: 'Guidelines', path: '/donor-guidelines' },
    { icon: faUser, label: 'Profile', path: '/user/profile' },
    { icon: faUser, label: 'About Us', path: '/userabout' },
    {icon:faMap, label:'Find Donor', path:'/fdonar' }
  ];
    ...
    <>
      ...
      <div className="sidebar" style={{ 
        ...
        <div className="menu-header" style={{ padding: '30px 25px 0 25px', marginBottom: '40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h1 style={{ fontSize: '1.8rem', fontWeight: 'bold', margin: 0, color: '#FFD700', textShadow: '0 0 10px rgba(255, 215, 0, 0.3)' }}>Blood Camp</h1>
            <p style={{ fontSize: '0.95rem', opacity: 0.8, marginTop: '5px', color: '#fff' }}>User Dashboard</p>
          </div>
          {isOpen && (
            <button
              onClick={() => setIsOpen(false)}
